...
namespace
{
	...
	SCP_string getJoystickGUID(SDL_Joystick* stick)
	{
		...
		return SCP_string(joystickGUID);
	}
...
void joy_device_changed(int state, int device)
{
	if (state == SDL_JOYDEVICEADDED)
	{
		...
		auto guid = getJoystickGUID(joystick);
...
int joy_init()
{
	...
	auto joystickGUID = getCurrentJoystickGUID();
	...
	if (joystickGUID != "")
	{
		for (int i = 0; i < n; ++i)
		{
			...
			if (joystick != nullptr)
			{
				auto currentGUID = getJoystickGUID(joystick);

				if (currentGUID == joystickGUID)
				{
					foundStick = joystick;
					break;
				}
	...
	else
	{
		...
		auto joystick = os_config_read_uint(NULL, "CurrentJoystick", 0);

		foundStick = SDL_JoystickOpen(joystick);
