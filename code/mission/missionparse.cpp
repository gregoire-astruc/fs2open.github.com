...
int get_mission_info(const char *filename, mission *mission_p, bool basic)
{
	...
	do {
		cfile::FileHandle *ftemp = cfile::io::open(real_fname);
		if (!ftemp) {
			rval = -1;
			break;
		}
		...
		filelength = cfile::io::fileLength(ftemp);
		cfile::io::close(ftemp);
		if (filelength == 0) {
			rval = -1;
			break;
		}
...
int parse_main(const char *mission_name, int flags)
{
	...
	do {
		...
		if (!(flags & MPF_IMPORT_FSM)) {
			cfile::FileHandle *ftemp = cfile::io::open(mission_name, cfile::MODE_READ, cfile::OPEN_NORMAL, cfile::TYPE_MISSIONS);
			...
			Current_file_length = cfile::io::fileLength(ftemp);
			cfile::io::close(ftemp);
		}
