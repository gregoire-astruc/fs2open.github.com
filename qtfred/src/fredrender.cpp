...
static const float FRED_DEFAULT_HTL_FOV = 0.485f;
static const float FRED_BRIEFING_HTL_FOV = 0.325f;
static const float FRED_DEAFULT_HTL_DRAW_DIST = 300000.0f;
...
static matrix	Last_control_orient = IDENTITY_MATRIX;
...
static void render_model_x_htl(vec3d *pos, grid *gridp, int col_scheme = 0);
static void draw_orient_sphere(object *obj, int r, int g, int b);
static void draw_orient_sphere2(int col, object *obj, int r, int g, int b);
static void render_compass(bool Render_compass);
static void draw_compass_arrow(vec3d *v0);
static void process_controls(vec3d *pos, matrix *orient, float frametime, int key, int mode = 0);
static void render_one_model(object *objp);
static void inc_mission_time();
static void draw_asteroid_field();
static void hilight_bitmap();
...
static const int FRED_COLOUR_WHITE = 0xffffff;
static const int FRED_COLOUR_YELLOW = 0x9fff00;

static void fred_enable_htl()
{
...
static void level_object(matrix *orient)
{
    vec3d u;
...
void level_controlled(const int viewpoint, const int view_obj)
{
    int cmode, count = 0;
    object *objp;
...
static void align_vector_to_axis(vec3d *v)
{
    float x, y, z;
...
static void verticalize_object(matrix *orient)
{
...
void verticalize_controlled(const int viewpoint, const int view_obj)
{
    int cmode, count = 0;
    object *objp;
...
static void render_waypoints(const waypoint_list * const cur_waypoint_list, const waypoint * const cur_waypoint)
{
    vertex v;
...
static vec3d* get_subsystem_world_pos2(object* parent_obj, ship_subsys* subsys, vec3d* world_pos)
{
    if (subsys == NULL) {
...
static int get_subsys_bounding_rect(object *ship_obj, ship_subsys *subsys, int *x1, int *x2, int *y1, int *y2)
{
    if (subsys != NULL) {
...
static void cancel_display_active_ship_subsystem(subsys_to_render &Render_subsys)
{
...
static void fredhtl_render_subsystem_bounding_box(subsys_to_render * s2r, subsys_to_render &Render_subsys)
{
...
static void display_active_ship_subsystem(subsys_to_render &Render_subsys, int cur_object_index)
{
...
static void render_one_model_htl(
...
static void render_one_model_nohtl(
    object *objp, int cur_object_index,
    bool Show_starts, bool Show_ships, bool Show_iff[],
    bool Bg_bitmap_dialog,
    bool Show_ship_models, bool Show_dock_points,
    bool Show_paths_fred);
static void render_one_model_briefing_screen(object *objp);

static void render_models(
    ...
    bool Show_paths_fred, bool Lighting_on, bool FullDetail)
{
    ...
    if (Cmdline_nohtl)
    {
        std::function<void (object *)> render_function(
            std::bind(&render_one_model_nohtl, std::placeholders::_1,
    ...
    else
    {
        fred_enable_htl();
        std::function<void (object *)> render_function(
            std::bind(&render_one_model_htl, std::placeholders::_1,
                cur_object_index, Show_starts, Show_ships, Show_iff,
...
static void render_model_x(vec3d *pos, grid *gridp, int col_scheme)
{
...
static void render_model_x_htl(vec3d *pos, grid *gridp, int col_scheme)
{
    vec3d	gpos;	//	Location of point on grid.
    vec3d	tpos;
    float	dxz;
...
static void render_active_rect(bool box_marking, const Marking_box &marking_box)
{
    if (box_marking) {
        gr_set_color(255, 255, 255);
...
static void process_movement_keys(int key, vec3d *mvec, angles *angs)
{
    int	raw_key;
...
static void process_controls(vec3d *pos, matrix *orient, float frametime, int key, int mode)
{
...
static void fred_render_grid(grid *gridp)
{
    int	i, ncols, nrows;
...
//	If not, return false.
static int object_check_collision(
...
static void render_compass(bool Show_compass)
{
    if (!Show_compass)
        return;
